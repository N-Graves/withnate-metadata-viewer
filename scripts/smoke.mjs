...
const JS = new URL("../dist/metadata-viewer.js", import.meta.url);
const CSS = new URL("../dist/metadata-viewer.css", import.meta.url);
...
const foreign = [...new Set(selectors.map((s) => s.trim()))].filter((s) => !s.startsWith(".mv"));
check(foreign.length === 0, "defines only .mv- classes", foreign.join(" "));
